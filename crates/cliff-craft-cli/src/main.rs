...
use cliff_craft_core::{sentinel_self_check, git::{open_repository}, crypto::generate_keypair};
...
enum Commands {
    ...
    /// Initialize cliff-craft in current repository
    Init {
        /// Repository path (default: current directory)
        #[arg(short, long, default_value = ".")]
        path: String,
    },
    /// Desactiva cliff-craft en el repositorio actual (elimina hooks)
    Disable {
        /// Repository path (default: current directory)
        #[arg(short, long, default_value = ".")]
        path: String,
    },
    /// Start the cliff-craft daemon
    Daemon {
...
async fn main() {
    ...
    match cli.command {
        ...
        Commands::Report { limit, format } => {
            ...
            match cliff_craft_core::git::get_governance_history(&repo, limit) {
                Ok(entries) => {
                    if format == "json" {
        ...
        Commands::Init { path } => {
            ...
            match open_repository(repo_path) {
                Ok(_repo) => {
                    println!("Initializing cliff-craft in: {}", path);
                    ...
                    // Add cliff-craft configuration to git config
                    // This would typically include the public key and other settings
                    
                    // Install hooks
                    match cliff_craft_core::git::install_hooks(&_repo) {
        ...
        Commands::Disable { path } => {
            ...
            match open_repository(repo_path) {
                Ok(repo) => {
                    match cliff_craft_core::git::remove_hooks(&repo) {
                        Ok(_) => println!("✅ Cliff-Craft disabled: Hooks removed successfully"),
        ...
        Commands::Daemon { config, daemon } => {
            if daemon {
                println!("Starting cliff-craft daemon in background with config: {}", config);
            } else {
                println!("Starting cliff-craft daemon with config: {}", config);
            }
            // Implementation would start the daemon process
            println!("✅ Daemon started successfully");
        }
        Commands::Status => {
            match query_daemon(cliff_craft_core::protocol::Request::GetStatus).await {
                Ok(cliff_craft_core::protocol::Response::Status { is_running, uptime_secs, events_captured }) => {
                    println!("Daemon Status:");
                    println!("  Running: {}", if is_running { "✅ Yes" } else { "❌ No" });
                    println!("  Uptime:  {}s", uptime_secs);
                    println!("  Events:  {}", events_captured);
                }
                Ok(cliff_craft_core::protocol::Response::Error(e)) => {
        ...
        Commands::On => {
            ...
            match Command::new("cliff-craft-daemon")
                .stdin(Stdio::null())
                .stdout(Stdio::null())
                .stderr(Stdio::null())
                .spawn() {
                    Ok(_) => {
                        ...
                        println!("Usa 'cliff-craft status' para verificar.");
                    },
                    Err(_) => {
                        // Reintento con path explícito por si no está en PATH todavía
                        match Command::new("/usr/local/bin/cliff-craft-daemon")
                            .stdin(Stdio::null())
                            .stdout(Stdio::null())
                            .stderr(Stdio::null())
                            .spawn() {
        ...
        Commands::Metrics { short } => {
            match query_daemon(cliff_craft_core::protocol::Request::GetMetrics).await {
                Ok(cliff_craft_core::protocol::Response::Metrics { 
        ...
        Commands::RegisterKey { key, alias } => {
            ...
            match cliff_craft_core::git::register_public_key(&repo, &key, &alias) {
        ...
        Commands::Verify { commit, format } => {
            use cliff_craft_core::git::{get_trusted_keys};
            use cliff_craft_core::crypto::{verify_signature, VerifyingKey};
            ...
            if !found {
                if format == "json" {
                    ...
                    eprintln!("❌ FALLO DE VERIFICACIÓN: No se encontró firma válida de Cliff-Craft.");
                }
                process::exit(1);
            }
        }
        Commands::VerifyWork => {
            use cliff_craft_core::git::get_staged_diff;
            ...
            match query_daemon(cliff_craft_core::protocol::Request::GetTicket { cost }).await {
                Ok(cliff_craft_core::protocol::Response::Ticket { success, message, signature }) => {
                    if success {
                        ...
                        match query_daemon(cliff_craft_core::protocol::Request::GetWitness { reset: true }).await {
                            Ok(cliff_craft_core::protocol::Response::Witness { data }) => {
                                let witness_file = repo.path().join("cliff-craft").join("latest_witness");
        ...
        Commands::Config { action } => {
            match action {
                ConfigAction::Init => {
                    let config_content = r#"# Cliff-Craft Configuration
...
"#;
                    let path = Path::new("cliff-craft.toml");
                    if path.exists() {
                        eprintln!("⚠️ cliff-craft.toml already exists!");
                        process::exit(1);
                    }
                    ...
                    println!("✅ Created cliff-craft.toml");
                }
                ConfigAction::Check => {
                    match cliff_craft_core::config::GovConfig::load() {
                        Ok(cfg) => {
...
async fn query_daemon(request: cliff_craft_core::protocol::Request) -> anyhow::Result<cliff_craft_core::protocol::Response> {
    use tokio::net::UnixStream;
    use tokio::io::{AsyncReadExt, AsyncWriteExt};

    let socket_path = "/tmp/cliff-craft.sock";
    ...
    let response: cliff_craft_core::protocol::Response = serde_json::from_slice(&buffer[..n])?;
    Ok(response)
}
async fn run_setup(no_confirm: bool) {
    println!("\n{} {} {}", style("===").blue(), style("Orquestador Soberano cliff-craft v2.1").bold(), style("===").blue());
    ...
    println!("Ejecuta {} para empezar a validar tu entropía.", style("cliff-craft on").cyan());
}
