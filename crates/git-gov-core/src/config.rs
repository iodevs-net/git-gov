...
impl GovConfig {
    pub fn load() -> Result<Self, config::ConfigError> {
        let builder = Config::builder()
            ...
            // Local config
            .add_source(File::with_name("git-gov").required(false))
            // Global config
            .add_source(File::with_name(&format!("{}/.config/git-gov/config", std::env::var("HOME").unwrap_or_else(|_| ".".into()))).required(false))
