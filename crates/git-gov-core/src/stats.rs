...
pub fn calculate_burstiness(times: &[f64]) -> f64 {
    ...
    // Burstiness = std_dev / mean (coeficiente de variación)
    // Aseguramos que el resultado sea finito y razonable
    let burstiness = std_dev / mean;
    
    // Validar que el resultado sea finito y no negativo
    if burstiness.is_finite() && burstiness >= 0.0 {
        burstiness.min(100.0)  // Límite superior razonable
    } else {
        0.0
    }
}
...
pub fn calculate_human_score(burstiness: f64, ncd: f64) -> f64 {
    // Normalizar burstiness (0-1 range)
    let norm_burstiness = burstiness.min(1.0);
    
    // NCD ya está en rango 0-1
    let norm_ncd = ncd.min(1.0);
...
mod tests {
    ...
    fn test_burstiness_calculation() {
        // Datos con alta variabilidad (humano-like)
        let human_times = vec![10.0, 0.1, 0.1, 20.0, 0.1, 0.1, 30.0];
        let burstiness = calculate_burstiness(&human_times);
        assert!(burstiness > 0.5, "High burstiness expected for human patterns");
        
        // Datos con baja variabilidad (AI-like)
        let ai_times = vec![1.0, 1.01, 0.99, 1.0, 1.0, 1.01];
        let burstiness = calculate_burstiness(&ai_times);
        assert!(burstiness > 5.0, "High burstiness expected for AI patterns");
    }
    ...
    fn test_human_score_calculation() {
        // Alto burstiness + bajo NCD = alto score humano
        let score = calculate_human_score(0.9, 0.2);
        assert!(score > 0.6, "High human score expected");
        
        // Bajo burstiness + alto NCD = bajo score humano
        let score = calculate_human_score(0.1, 0.8);
        assert!(score < 0.4, "Low human score expected");
    }
